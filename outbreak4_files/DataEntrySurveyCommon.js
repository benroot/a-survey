...
function doFieldEmbedding()
{
    ...
    $('#questiontable .rc-field-embed').each(function ()
    {
        ...
        $("input[type!='hidden']", source_data.parent()).each(function (i, e) {
            ...
            if (type == 'text' && !$(this).hasClass('hasDatepicker')) $(this).css({'max-width': '380px'});
            if (type === 'textarea' || (type == 'text' && !$(this).hasClass('hasDatepicker'))) $(this).css('width', '95%');
        });
...
function evalLogic(this_field, byPassEraseFieldPrompt, logic, parentField) {
    ...
    if (logic == false) {
        ...
        $(function(){
            $('#questiontable #'+this_field+'-tr .rc-field-embed').each(function(){
                evalLogic($(this).attr('var'), byPassEraseFieldPrompt, false, this_field);
            });
        });
    } else {
        // SHOW ROW
